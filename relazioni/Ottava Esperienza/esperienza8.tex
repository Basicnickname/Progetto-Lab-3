...
\end{table}
\subsubsection{Studio della mutarotazione del glucosio}
Con i dati raccolti è stato eseguito un fit esponenziale con la formula tipica delle reazioni del primo ordine: $\alpha = (\alpha_0 - \alpha_{inf}).\exp^{-kt} + \alpha_{inf}$, dai parametri si sono ricavati i valori $k_0$ e $k_{inf}$. Inoltre al raggiungimento dell'equilibrio sono stati misurati gli angoli di rotazione alle varie lunghezze d'onda per calcolare il potere rotatorio specifico $k$ a $\lambda = 589 \unit{nm}$ come nel passo precedente.
\begin{table}[H]
    \centering
    \begin{tabular}{
        l
        ...
        \midrule
        $k_0$ [\unit{deg.cm^2.g^-1}] & 11.20 & 11.03 & 0.71 & 6.45 \\
        $k_{inf}$ [\unit{deg.cm^2.g^-1}] & 5.23 & 5.35 & 0.63 & 11.81 \\
        \bottomrule
    \end{tabular}
...
\begin{table}[H]
    ...
    \begin{tabular}{
        ...
        \midrule
        Saccarosio + HCl & 7.46 & 8.43(1.51) & -2.18 & -1.77(0.47) \\     %T:errori un po' alti ma gli ho dovuti propagare a mano, non sono sicuro di quello sulla bilancia ma comunque non so se si può fare molto meglio
        Saccarosio (riferimento) & 7.46 & 8.04(1.45) & 7.46 & 7.26(1.33) \\
        \bottomrule
    \end{tabular}
...
della massa disciolta $P$; il valore ottenuto per saccarosio a $\lambda = 525 \unit{nm}$ risulta coerente con quello misurato nella parte successiva (\autoref{fig:2}) ed in accordo con il valore tabulato (\autoref{tab:Potere rotatorio specifico}). 
...
36[\%] $\alpha$-D-glucosio e 64[\%] $\beta$-D-glucosio, i valori dell'angolo di rotazione variano seguendo una cinetica esponenziale, trattandosi di una reazione del primo ordine (\autoref{fig:3}). Inoltre il valore misurato del potere rotatorio specifico all'equilibrio risulta coerente con quello tabulato (\autoref{tab:Potere rotatorio specifico mutarotazione}).
%Forse bisogna aggiungere qualcosa circa al correzione per glucosio monoidrato (per ora non l'ho fatta)
...
sono in accordo con i valori tabulati per entrambe le soluzioni entro $3\sigma$ (\autoref{tab:Potere rotatorio specifico zucchero invertito}).
