...
%T: in generale aggiustare il format come le altre relazioni, su github i riferimenti
%   attenzione che probabilmente i warning sono perché di base non si può usare la notazione scientifica nelle didascalie, titoli, etc., vanno corretti tipo con \texorpdfstring{}, gemini lo sa meglio
% Abstract
%T: ridurre l'abstract e magari fare un elenco puntato che divida in grande le fasi
\begin{abstract}
...
%T: questa parte può essere molto utile per studiare quindi se riesci copiala da un'altra parte ma se ho capito bene il prof non la vuole proprio (al massimo qualche breve commento alla fine se significativo per spiegare qualche misura)
...
\end{equation}
dove il termine $I_{ph}$  è proporzionale al numero di fotoni assorbiti, ovvero all’intensità della radiazione elettromagnetica.
La fotocorrente generata nel fotodiodo è proporzionale alla luce emessa dal LED e quindi alla corrente di alimentazione
\begin{equation}
    I_{ph} = \beta I_{LED}
\end{equation}
non bisogna usare la tensione di alimentazione perché la caratteristica corrente--tensione del LED è di tipo non lineare.
...
In questo modo è più facile notare che in presenza di polarizzazione inversa ($V_g < 0$) con un valore assoluto sufficientemente grande, il termine esponenziale legato alla corrente di buio nell'equazione del diodo diventa trascurabile.
...
Per fittare i dati è opportuno scambiare gli assi e scrivere invece $I_{ph}$ in funzione di $V_R$.
Introduciamo anche il fattore di idealità n, che tipicamente è compreso fra 1 e 5 ed è legato ai processi di ricombinazione non radiativa nel diodo. 
\begin{equation}
...
%T: fare una subsection per materiali e una per metodi, nei materiali non serve descrivere troppo gli strumenti (tipo resistenze dire solo 10 e 20 k)
\begin{itemize}
...
%T: non penso serva descrivere il setup, bastano le descrizioni dei circuiti prima
\begin{figure}[H]
  \centering
...
%T: unire questa parte a quella dei metodi sopra che descrive come sono state fatte le misure, stringere un po' e lasciare solo le formule usate per fittare, queste ultime eventualmente possono essere messe prima dei fit
...
%T: non serve mettere tutte le misure esplicitamente, mettere solo 2 plot uno in scala lineare ed una semilogaritmica e mettere tutte le curve con la legenda che specifica il tipo di misura (vedi le nostre e i colab come reference)
%   per i grafici togliere l'indice del tipo di scala usato da python ma specificarlo nella caption
\begin{table}[H]
    \centering
...
\begin{figure}[H]
  \centering
  \includegraphics[width=0.8\textwidth]{R10Vg03.png}
...
\begin{table}[h]
%T: unità di misura? ridurre mettere solo 2 massimo 3 cifre decimali e una cifra di errore, idem errore relativo
\centering
...
In cui $\beta_1$, $\beta$, sono stati ricavati dai dati presi rispettivamente per  $R = 10\text{ k}\Omega$ e $V_g=0.3$ e $R = 20\text{ k}\Omega$ e $V_g=1$.
...
%T: usare pedici e apici sui parametri per far capire a quali misure corrispondono, lasciare le spiegazioni solo per metodi o conclusioni
  \begin{equation}
...
\section{Discussion}
%T: stringere un po': limitarsi a dire quali misure tornano e eventualmente quali no e perché 
% occhio ad usare troppa AI che potrebbero sgamarci, rielaborare il più possibile
...
I coefficienti $\beta$ presentano errori relativi contenuti nell'intervallo $2-3\%$.
 ...
 Questa discrepanza è riconducibile alla presenza di fenomeni di ricombinazione non radiativa all'interno della regione di svuotamento del semiconduttore.
  Il valore ottenuto, dell'ordine di {$10^{-9}$}{A}, rientra nel range tipico atteso per i dispositivi LED ad ampio band-gap ($10^{-9} - 10^{-11}$\,A). Tuttavia, l'errore relativo associato risulta estremamente elevato.
