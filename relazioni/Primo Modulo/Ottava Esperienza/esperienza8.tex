...
\end{table}
\subsubsection{Studio della mutarotazione del glucosio} %   F: Nella formula \alpha_0 - \alpha_{\infty}) \cdot \exp^{-ct} + \alpha_{\infty} l'esponenziale si può anche scrivere semplicemente con e^{-ct}, ma di nuovo solo per bellezza, per non avere la scritta exp nella formula
Con i dati raccolti è stato eseguito un fit esponenziale con la formula tipica delle reazioni del primo ordine: $\alpha = (\alpha_0 - \alpha_{\infty}) \cdot e^{-ct} + \alpha_{\infty}$, dai parametri si sono ricavati i valori $k_0^{525}$ e $k_{\infty}^{525}$ per una soluzione di glucosio in luce verde.
...
sono in accordo con i valori tabulati entro $3\sigma$ (\autoref{tab:Potere rotatorio specifico zucchero invertito}). 
\end{document}